...
export const ChatTextArea = ({adventureStarted, setAdventureStarted, setPrompt, prompt}: ChatTextAreaProps) => {
  ...
  const fetchChat = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    - There is an existing story in the following chat logs. Please refer to it for setting, characters, previous choices made, and any other directions.
    ...
    // - When you are done with your response make a new line below the choices, print "IMAGE_DESCRIPTION".
    // - Create another new line and then describe the current scene in a way that an AI image generator could generate an image for this scene in the style of a cartoon
  ...
  return (
    <div className='w-11/12 mx-auto flex flex-col'>
      ...
      <textarea className='text-black  my-10 border-gray-500 border-2 p-8' rows={15} cols={10} value={chatResponse.length ? chatResponse : adventureStarted ? 'LOADING...' : ''} readOnly/>
    </div>
  );
};
